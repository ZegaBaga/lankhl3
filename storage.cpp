...
void __fastcall TFormStorage::FormCreate(TObject *Sender)
{
	...
	StringGrid1->Cells[1][0]="Тип оборудования";
	StringGrid1->Cells[2][0]="Название модели";
	StringGrid1->Cells[3][0]="Серийный №";
	StringGrid1->Cells[4][0]="Инвентарный №";
	StringGrid1->Cells[5][0]="Дата выпуск";
...
void __fastcall TFormStorage::SpeedButton1Click(TObject *Sender)
{
	...
	if(mr==mrOk)
	{
		 ...
		 //ADOQuery1->SQL->Add("insert into storage(tipname,equipname) values('Принтер','HP LaserJet')");
		 ADOQuery1->ExecSQL();
		 ADOQuery1->Close();
...
void __fastcall TFormStorage::SpeedButton3Click(TObject *Sender)
{
	...
	UnicodeString qu=L"Уверены что, хотите удалить "+tip+L":"+fname;
	UnicodeString cap=L"Удаление";
